...
By default, all four servers are started as daemons when the Raspberry Pi is turned on. This is done with a default
...
them to the `/etc/init.d` directory:
    ...
    sudo cp init.d/eostouchd /etc/init.d
...
Then add them to the default run level, so they get started at boot.
    ...
    sudo update-rc.d eostouchd defaults
